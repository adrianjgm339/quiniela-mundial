...
export class AdminGroupsService {
  ...
  private teamName(team: any, locale: string) {
    const t =
      team?.translations?.find((x: any) => x.locale === locale) ??
      team?.translations?.[0];
  ...
  private keyH2H(
    stats: Map<TeamId, { pts: number; gf: number; ga: number }>,
    teamId: string,
  ): Key {
  ...
  private orderGroupFifa(rows: TeamRow[], matches: MatchRow[]): TeamRow[] {
    ...
    const base = [...rows].sort((a, b) =>
      this.compareKeyDesc(this.keyGlobal(a), this.keyGlobal(b)),
    );
  ...
  private orderGroupWbc(rows: TeamRow[], matches: MatchRow[]) {
    ...
    while (i < base.length) {
      ...
      while (
        j < base.length &&
        Math.abs(winPct(base[j]) - winPct(base[i])) < 1e-12
      )
        j++;
  ...
  private resolveTieBlockWbc(block: TeamRow[], matches: MatchRow[]) {
    ...
    if (swept || sweptByAll) {
      const middle = block.filter(
        (t) => t.teamId !== swept?.teamId && t.teamId !== sweptByAll?.teamId,
      );
      const resolvedMiddle =
        middle.length <= 1 ? middle : this.resolveTieBlockWbc(middle, matches);
    ...
    for (let i = 0; i < sorted.length;) {
      ...
      while (
        j < sorted.length &&
        Math.abs(raQ(sorted[j].teamId) - raQ(sorted[i].teamId)) < 1e-12
      )
        j++;
  ...
  private resolveTieBlock(block: TeamRow[], matches: MatchRow[]) {
    ...
    const withH2H = [...block].sort((a, b) =>
      this.compareKeyDesc(
        this.keyH2H(h2h, a.teamId),
        this.keyH2H(h2h, b.teamId),
      ),
    );
    ...
    for (const [, g] of groups) {
      ...
      const subSorted = [...g].sort((a, b) =>
        this.compareKeyDesc(
          this.keyH2H(subH2H, a.teamId),
          this.keyH2H(subH2H, b.teamId),
        ),
      );
  ...
  private async resolveSeasonId(userId: string, seasonId?: string) {
    ...
    if (!u?.activeSeasonId)
      throw new BadRequestException('seasonId is required (no active season)');
    return u.activeSeasonId;
  }

  async computeStandings(args: {
    userId: string;
    seasonId?: string;
    locale: string;
  }) {
    ...
    for (const m of matches) {
      ...
      tm.set(m.homeTeamId, {
        id: m.homeTeamId,
        name: this.teamName(m.homeTeam, locale),
      });
      tm.set(m.awayTeamId, {
        id: m.awayTeamId,
        name: this.teamName(m.awayTeam, locale),
      });
    ...
    for (const groupCode of groupCodes) {
      ...
      result.push({
        ...
        expectedMatches,
        isComplete:
          expectedMatches > 0 ? gMatches.length === expectedMatches : false,
  ...
  async computeThirds(args: {
    ...
    if (thirds.length >= 9) {
      ...
      const k9 = thirds[8];
      needsManualCut =
        k8.points === k9.points && k8.gd === k9.gd && k8.gf === k9.gf;
    ...
    try {
      ...
      if (persisted.length > 0) {
        const map = new Map<string, any>(
          persisted.map((p: any) => [String(p.teamId), p]),
        );
        const anyOverride = persisted.some(
          (p: any) => !!p.manualOverride || !!p.manualReason,
        );
  ...
  async resolveKoPlaceholders(args: {
    userId: string;
    seasonId?: string;
    locale: string;
  }) {
  ...
  async closeGroups(args: {
    ...
    const incomplete = standings.groups
    ...
    if (incomplete.length > 0) {
      throw new BadRequestException(
        `Cannot close group stage: incomplete groups: ${incomplete
          .map(
            (x) =>
              `${x.groupCode}(${x.confirmedMatches}/${x.expectedMatches ?? '?'})`,
          )
          .join(', ')}`,
      );
    }
    ...
    let bracketSeed: any = {
      created: 0,
      skipped: true,
      reason: 'bracket disabled for this sport',
    };
    ...
    const needsManualGroups = standings.groups
      .filter((g: any) => (g.standings ?? []).some((r: any) => r.needsManual))
      .map((g: any) => g.groupCode);

    const completeGroups = standings.groups
      .filter((g: any) => g.isComplete)
      .map((g: any) => g.groupCode);
  ...
  async setManualGroupOrder(args: {
    userId: string;
    dto: {
      seasonId: string;
      groupCode: string;
    ...
    const orderedTeamIds = (args.dto.orderedTeamIds ?? []).map((x) =>
      String(x),
    );
    if (!seasonId || !groupCode)
      throw new BadRequestException('seasonId and groupCode are required');
    ...
    if (expectedCount <= 0) {
      throw new BadRequestException(
        `No teams found for seasonId=${seasonId} groupCode=${groupCode}`,
      );
    }

    if (orderedTeamIds.length !== expectedCount) {
      throw new BadRequestException(
        `orderedTeamIds must have exactly ${expectedCount} teamIds`,
      );
    }
    ...
    for (const id of orderedTeamIds) {
      if (!validIds.has(id))
        throw new BadRequestException(
          `teamId not in group ${groupCode}: ${id}`,
        );
    }
  ...
  private async applyGroupPlaceholdersToKOMatches(opts: {
    ...
    const resolveFromPlaceholderRule = (
      rule?: string | null,
    ): string | null => {
      if (!rule) return null;
    ...
    const koMatches = await this.prisma.match.findMany({
      ...
      select: {
        ...
        awayTeamId: true,
        homeTeam: {
          select: { id: true, isPlaceholder: true, placeholderRule: true },
        },
        awayTeam: {
          select: { id: true, isPlaceholder: true, placeholderRule: true },
        },
  ...
  async setManualThirds(args: {
    ...
    if (qualified.length !== 8)
      throw new BadRequestException(
        'qualifiedTeamIds must have exactly 8 teamIds',
      );

    // 1) Calcular terceros "live" (orden correcto) y sincronizar thirdPlaceRanking
    const computed = await this.computeThirds({
      userId: args.userId,
      seasonId,
      locale: 'es',
    });
    const computedThirds = computed.thirds ?? [];
    const computedIds = computedThirds.map((t) => String(t.teamId));

    if (computedIds.length === 0) {
      throw new BadRequestException(
        'No third-place teams found from live standings for this seasonId.',
      );
    }
    ...
    for (const id of qualified) {
      if (!thirdSet.has(id))
        throw new BadRequestException(
          `teamId is not in thirdPlaceRanking: ${id}`,
        );
    }
    ...
    if (tieIds.size > 0) {
      ...
      for (const id of fixedAutoQualified) {
        if (!qualified.includes(id)) {
          throw new BadRequestException(
            `Cannot exclude fixed auto-qualified teamId: ${id}`,
          );
        }
      }
      ...
      for (const id of qualified) {
        if (!fixedAutoQualified.has(id) && !tieIds.has(id)) {
          throw new BadRequestException(
            `Invalid selection outside tie block: ${id}`,
          );
        }
      }
    }
  ...
  private isThirdComboPlaceholder(
  ...
  private parseAllowedGroupsFromPlaceholder(
    placeholderText: string | null | undefined,
  ): string[] {
    const raw = (placeholderText ?? '').trim();
    if (!raw) return [];
  ...
  private async autoAssignThirdsToBracketSlots(args: {
    ...
    if (!Array.isArray(slots) || slots.length === 0)
      return { updatedCount: 0, unresolvedCount: 0 };
    if (!Array.isArray(eligibleThirds) || eligibleThirds.length === 0)
      return { updatedCount: 0, unresolvedCount: 0 };
    ...
    for (const slot of candidates) {
      const allowed = this.parseAllowedGroupsFromPlaceholder(
  ...
  private async applyR32BracketSlotsToMatches(args: {
    seasonId: string;
    matchNo?: number;
  }) {
    ...
    for (const [mNo, pair] of byMatch.entries()) {
      ...
      if (pair.home?.teamId) {
        ...
        const pid = placeholderMap.get(
          (pair.home.placeholderText ?? '').trim(),
        );
      ...
      if (pair.away?.teamId) {
        ...
        const pid = placeholderMap.get(
          (pair.away.placeholderText ?? '').trim(),
        );
